...
public class CreateNewProblem extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
     ...
     response.setContentType("text/html;charset=UTF-8");
            ...
            problem.setYield(request.getParameter("yield"));
            problem.setDescription(request.getParameter("description"));
            problem.setStatus("P");
          
           
            boolean test = recDB.createNewProblem(problem);
            if(test){
                ServletContext context = getServletContext();
                RequestDispatcher rd = context.getRequestDispatcher("/ViewThresholds");
